  ...
  */
  ...
  /**
    ...
    Yes, as there are multiple ways to trigger the fallback function. We can send a transaction with calldata
    that specifies a function that does not exist in the original contract, e.g., 

    sendTransaction({
      from: player,
      to: contract.address,
      value: toWei('0.0000001','ether') 
      data: web3.eth.abi.encodeFunctionSignature('doesNotExist()')
    })

    it will also trigger `receive()` and give us the ownership.
  */
  ...
  */
  ...
  /**
    ...
    I could call `sendAllocation` as many times as I want and get all the funds.
  */
  ...
  /**
    4. What could the programmer have done differently to avoid this vulnerability?
    Programmers can choose to declare constructors using the `constructor` keyword 
    when possible just to be careful, e.g., 
    ...
    Remember to reset each contributor's amount to 0 before initiating the transfer, e.g.,

    function sendAllocation(address payable allocator) public {
      require(allocations[allocator] > 0);
      value = allocations[allocator];
      allocations[allocator] = 0;
      allocator.transfer(value);
    }
  ...
  */
  ...
  /**
    3. Is there another series of calls/a different exploit contract that would have also led to an exploit?
    Yes, as long as there are multiple layers in between of the victim contract and the original transaction 
    caller. E.g., caller -> malicious contract A -> malicious contract B -> victim contract.
  */

  /**
    4. What could the programmer have done differently to avoid this vulnerability?
    Programmers should have a clear understanding of tx.origin and msg.sender and be aware that your
    your contract might be interacting with another contract and that msg.sender can be associated 
    with a contract, not necessarily an individual account.

    Thus, avoid using equal/unequal relationship bewteen tx.origin and msg.sender to judge if it
    is from the same individual. Instead, we can use a modifier onlyOwner to the changeOwner() method
    so that only the current owner can transfer his/her ownership to others.
  ...
  */
  ...
  /**
    3. Is there another series of calls/a different exploit contract that would have also led to an exploit?
    Yes, we could exploit it to its maximum.
    ...
    contract TokenAttack {
        function attack1(address victimContract) public {
            TokenInterface(victimContract).transfer(msg.sender, 1);
        }

        function attack2(address victimContract) public {
            TokenInterface(victimContract).transfer(msg.sender, 2**256 - 1);
        }
    }
  */
  ...
  */
  ...
  /**
    3. Is there another series of calls/a different exploit contract that would have also led to an exploit?
    Inside receive(), we can use some other ways to reject fund transfer from the original contrat, say, 
    having an infinite loop there:

    receive() external payable {
      while (true) {}
    }

    so that the transaction will run out of gas and fail every time, preventing the original contract owner
    to reclaim the king.
  ...
  */
  ...
  /**
    2. What is the difference between the Rinkeby and Georli networks? Did you notice any differences?
    Rinkeby's maximum gas fee can be a lot higher than Goerli's, even though their base gas fee is
    roughly the same. Rinkeby's max gas fee can be higher than 1.5 GWei whereas Goerli's rarely exceeds
    0.2 GWei.

    Also, it seems that users can get more Rinkeby test tokens, e.g., at least 8 ETH at a time, 
    whereas for Goerli, we can only get 0.5 ETH for every 24 hours.
  */

  /**
    3. What is the Rubixi bug?
    The company changed its contract name from DynamicPyramid to Rubixi but forgot to update it in
    the constructor, which has enabled anyone to call the constructor like a normal function. The
    attacker called the constructor, claimed ownership and stole the funds.
    */
